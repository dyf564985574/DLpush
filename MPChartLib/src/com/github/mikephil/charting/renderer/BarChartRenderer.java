...
public class BarChartRenderer extends DataRenderer {
    ...
    protected void drawDataSet(Canvas c, BarDataSet dataSet, int index) {
        spacing = mChart.getBarData().getSpacing();
        ...
        if (dataSet.getColors().size() > 1) {
            ...
            for (int j = 0; j < buffer.size(); j += 4) {
                ...
                if (mChart.isDrawBarShadowEnabled()) {
                    //修改
                    Log.i("isDrawBarShadowEnabled","左"+buffer.buffer[j]+"  上"+mViewPortHandler.contentTop()+"  右"+buffer.buffer[j + 2]+"  下"+mViewPortHandler.contentBottom());
            ...
            for (int j = 0; j < buffer.size(); j += 4) {
                ...
                if (mChart.isDrawBarShadowEnabled()) {
                    ...
                    c.drawRect(buffer.buffer[j] + spacing , mViewPortHandler.contentTop()+YAxisRenderer.index*10,
                            buffer.buffer[j + 2] - spacing,
                            mViewPortHandler.contentBottom(), mShadowPaint);
                }

                c.drawRect(buffer.buffer[j] +spacing, buffer.buffer[j + 1], buffer.buffer[j + 2] -spacing,
                        buffer.buffer[j + 3], mRenderPaint);
            }
        }
    }
