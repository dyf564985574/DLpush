...
import com.toc.dlpush.index.manager.ManagerCaringActivity;
import com.toc.dlpush.index.manager.ManagerNoticesActivity;
import com.toc.dlpush.index.manager.ManagerProposeActivity;
import com.toc.dlpush.index.manager.ManagerSuggestActivity;
import com.toc.dlpush.index.manager.ManagerUserActivity;
...
/**
 * Created by Yangweisi on 2015/6/13.
 */
public class ManagerActivity extends FragmentActivity implements View.OnClickListener,BackHandledInterface {
    private SlidingMenu mMenu;
    protected static final String TAG = "MainActivity";
    private Context mContext;
    private Button mConstact, mDeynaimic, mSetting ,mUser;
    ...
    private  RelativeLayout relat_add;//添加联系人的相对布局
    private  RelativeLayout relat_revise;//修改密码的相对布局
    private  RelativeLayout relat_update;//检查更新的相对布局
    private  RelativeLayout relat_function;//功能接收的相对布局
    private  RelativeLayout relat_clear;//清除缓存的相对布局
    private  RelativeLayout relat_logoff;//退出当前账号的相对布局
    private  String uid = "";
    private  int index = 0;
    private  Updatas updatase;
    private String firstLogin;
    private ProgressDialog pd;
    private  int first=1;//判断是不是第一次进入
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case DOWNLOAD_SUCCESS:
                    ...
                    break;
                case DOWNLOAD_ERROR:
                    UtilTool.showToast(ManagerActivity.this, "下载apk失败");
                    pd.dismiss();
                    break;
                case UPDATE_ERROR:
                    UtilTool.showToast(ManagerActivity.this, "此版本是最新版本");
                    break;
                case NEED_UPDATE:
                    // 从服务器下载最新的apk安卓
                    if(msg.obj==null){
...
//		intent.setData(Uri.fromFile(file));
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        index = it.getIntExtra("index01", 0);

        mMenu = (SlidingMenu) findViewById(R.id.manager_menu);
        uid = UtilTool.getSharedPre(ManagerActivity.this, "users", "uid", "0");
        firstLogin=UtilTool.getSharedPre(ManagerActivity.this, "users", "firstLogin01", "0");
        findView();
        init();
    ...
    private void findView() {
        ...
        if("0".equals(firstLogin)){
            ...
            firstLogin="1";
            UtilTool.setSharedPre(ManagerActivity.this,"users","firstLogin01",firstLogin);
        }
        ...
        buttomBarGroup = (LinearLayout) findViewById(R.id.manager_bar_group);
        mNews = (Button) findViewById(R.id.manager_tip);
        mConstact = (Button) findViewById(R.id.manager_suggest);
        mSetting = (Button) findViewById(R.id.manager_propose);
        mUser = (Button) findViewById(R.id.manager_user);
    ...
    private void init() {
        ...
        mUser.setOnClickListener(usersOnClickListener);
        if (index == 0) {
            mNews.performClick();
        } else if (index == 1) {
    ...
    private View.OnClickListener newsOnClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            ManagerNoticesActivity newsFatherFragment = new ManagerNoticesActivity();
            ft.replace(R.id.manager_content, newsFatherFragment, MainActivity.TAG);
            ft.commit();
            setButton(v);
            currentTab = 0;
    ...
    private View.OnClickListener constactOnClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            ManagerSuggestActivity constactFatherFragment = new ManagerSuggestActivity();
            ft.replace(R.id.manager_content, constactFatherFragment, MainActivity.TAG);
            ft.commit();
            setButton(v);
            currentTab = 1;
    ...
    private View.OnClickListener settingOnClickListener = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            ManagerProposeActivity dynamicFragment = new ManagerProposeActivity();
            ft.replace(R.id.manager_content, dynamicFragment, MainActivity.TAG);
            ft.commit();
            setButton(v);
            currentTab = 2;
    ...
    private View.OnClickListener usersOnClickListener = new View.OnClickListener() {
        @Override
        public void onClick(View v) {


            FragmentTransaction ft = obtainFragmentTransaction(3);
            ManagerUserActivity newsFatherFragment = new ManagerUserActivity();
            ft.replace(R.id.manager_content, newsFatherFragment, MainActivity.TAG);
            ft.commit();
            setButton(v);
            currentTab = 3;
    ...
    public FragmentTransaction obtainFragmentTransaction(int index) {
        ...
        //如果是第一次进入ManagerActivity界面则不需要切换效果
        if(first!=1) {
            if (index > currentTab) {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.relat_add:
                Intent it = new Intent(ManagerActivity.this, AddActivity.class);
                startActivity(it);
                break;
            case R.id.relat_revise:
                Intent it01 = new Intent(ManagerActivity.this, ReviseActivity.class);
                startActivity(it01);
                break;
            case R.id.relat_logoff:
                ...
                if (networkInfo == null || !networkInfo.isAvailable()) {
                    UtilTool.showToast(ManagerActivity.this, Constantes.NETERROR);
                } else {
                    Dialog();
                }
                break;
            ...
            case R.id.relat_clear:
                DialogClear();
                break;case R.id.relat_function:
                Intent it02=new Intent(ManagerActivity.this, AboutsActivity.class);
                startActivity(it02);
                break;
    ...
    public class Logout extends AsyncTask<Void, Void, Return> {
        ...
        protected void onPostExecute(Return aReturn) {
            ...
            if ((null == aReturn.getError()) || ("1".equals(aReturn.getError()))) {
                UtilTool.showToast(ManagerActivity.this, Constantes.NETERROR);
                UtilTool.setSharedPre(ManagerActivity.this,"users","auto","1");
            } else {
//                DataCleanManager.cleanApplicationData(ManagerActivity.this);
                UtilTool.setSharedPre(ManagerActivity.this, "users", "auto", "1");


            }
            UtilTool.ClearData(ManagerActivity.this);
            Intent it02 = new Intent(ManagerActivity.this, LoginActivity.class);
            startActivity(it02);
            finish();
        }
    ...
    public class Updata extends AsyncTask<Void, Void, Updatas> {
        ...
        protected void onPostExecute(Updatas updatas) {
            ...
            if((null==updatas.getError())||("1".equals(updatas.getError()))){
                UtilTool.showToast(ManagerActivity.this,"网络异常");
            }else{
                //判断版本号是否相同
                if(getVersion().equals(updatas.getVersion())){
                    UtilTool.showToast(ManagerActivity.this,"已是最新版本");
...
//                DataCleanManager.cleanApplicationData(ManagerActivity.this);
                UtilTool.ClearData(ManagerActivity.this);
                UtilTool.showToast(ManagerActivity.this, "清除缓存成功");
            }
        });
        builder.setNegativeButton("取消", new DialogInterface.OnClickListener() {
            @Override
    ...
    */
    ...
    public  void DialogBack(){
        ...
        builder.setMessage("您确定要退出自贸区用电？");
