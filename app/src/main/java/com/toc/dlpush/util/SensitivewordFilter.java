...
public class SensitivewordFilter {
    ...
    public static   Boolean  GetSensitive(String args,Context context) {
        SensitivewordFilter filter = new SensitivewordFilter(context);

        System.out.println("待检测语句字数：" + args.length()+"   "+args);
        if(args.contains("艹")){
            return true;
        }
        long beginTime = System.currentTimeMillis();
        Set<String> set = filter.getSensitiveWord(args, 1);
